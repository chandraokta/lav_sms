...
class MarkController extends Controller
{
    ...
    public function selector(MarkSelector $req)
    {
        ...
        return redirect()->route('nilai.manage', [
            $req->exam_id, 
            $req->my_class_id, 
            $req->section_id, 
            $req->subject_id
    ...
    public function update(Request $req, $exam_id, $class_id, $section_id, $subject_id)
    {
        ...
        /** Process manual jenis nilai input **/
        foreach($marks->sortBy('user.name') as $mk)
        {
            $all_st_ids[] = $mk->student_id;

            // Get jenis nilai and nilai from request
            $jenis_nilai = $mks['jenis_nilai_'.$mk->id] ?? null;
            $deskripsi = $mks['deskripsi_'.$mk->id] ?? null;
            $nilai = $mks['nilai_'.$mk->id] ?? null;

            // Map jenis nilai to existing fields
            if ($jenis_nilai) {
                switch ($jenis_nilai) {
                    case 'uh':
                        $d['t1'] = $nilai;
                ...
                // Store jenis nilai and description
                $d['jenis_nilai'] = $jenis_nilai;
                $d['deskripsi'] = $deskripsi;
            } else {
                // If no jenis nilai selected, use default values
                $d['t1'] = 0;
                $d['t2'] = 0;
                $d['tca'] = 0;
                $d['exm'] = 0;
        ...
        $d3['total'] = $this->mark->getExamTotal($exam->term, $all_st_ids, $class_id, $subject_id, $this->year);
        $d3['ave'] = $this->mark->getExamAverage($d3['total'], $marks->count());
        $d3['class_ave'] = $this->mark->getClassAverage($exam_id, $class_id, $subject_id, $this->year);
        $d3['pos'] = $this->mark->getExamPosition($d3['total'], $exam_id, $class_id, $subject_id, $this->year);
        $d3['sub_pos'] = $this->mark->getSubjectPosition($d3['total'], $exam_id, $class_id, $subject_id, $this->year);
        $d3['year'] = $this->year;
    ...
    public function bulk($class_id = NULL, $section_id = NULL)
    {
        ...
        return view('nilai.create', $d);
    }

    public function bulk_select(Request $req)
    {
        return redirect()->route('nilai.create', [$req->my_class_id, $req->section_id]);
    }
    ...
    public function tabulation($exam_id = NULL, $class_id = NULL, $section_id = NULL)
    {
        ...
        return view('nilai.tabulation', $d);
    }
    ...
    public function print_tabulation($exam_id, $class_id, $section_id)
    {
        ...
        return view('nilai.print_tabulation', $d);
    }
    ...
    protected function verifyStudentExamYear($student_id, $year = null)
    {
        ...
        if(!$year){
            if($student_exists && $years->count() > 0)
            {
                ...
                return view('nilai.select_year', $d);
            }
